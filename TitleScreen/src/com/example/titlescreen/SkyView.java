...
public class SkyView extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// RelativeLayout.LayoutParams LD = new
		...
		RelativeLayout.LayoutParams ZO = new RelativeLayout.LayoutParams(
		...
		zoomout.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				if (!loadingFlag && ZoomLevel > 0
						&& System.currentTimeMillis() - ZoomTimer > 50) {
					TextView textView = (TextView) findViewById(12345);
					ZoomLevel -= 10;
					textView.setText("Zoom: " + ZoomLevel);
					cam.moveCamera(Camera.CAMERA_MOVEOUT, 10);
		...
		RelativeLayout.LayoutParams ZI = new RelativeLayout.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
		...
		tv.setId(12345);
		RelativeLayout.LayoutParams ZT = new RelativeLayout.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
		...
		CS.setId(23456);
		RelativeLayout.LayoutParams CSP = new RelativeLayout.LayoutParams(
				LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
	...
	public boolean onTouchEvent(MotionEvent me) {
		if (!loadingFlag) {
			...
			/*
			 * if(me.getAction() == MotionEvent.ACTION_POINTER_DOWN) { Id2 =
			 * me.getActionIndex(); float xd = me.getX(Id2); float yd =
			 * me.getY(Id2); float xdist = xd - xpos; float ydist = yd - ypos;
			 * mode = "zoom"; if(xdist*xdist + ydist*ydist > 10) { //mode =
			 * "zoom"; } return true; }
			 * 
			 * if(me.getAction() == MotionEvent.ACTION_POINTER_UP) { mode =
			 * "drag"; return true; }
			 */
	...
	class MyRenderer implements GLSurfaceView.Renderer {
		...
		public void DrawConstellation(Constellation Con, World Sky) {
			...
			ArrayList<SimpleVector> StarVectors = new ArrayList<SimpleVector>(
					StarList.size());
			...
			int yAxisRotation = SkySpace.GetXrot(RotValues.first,
					RotValues.second);
			int xAxisRotation = SkySpace.GetYrot(RotValues.first,
					RotValues.second);
		...
		public void onSurfaceChanged(GL10 gl, int w, int h) {
			...
			if (master == null) {
				...
				// Create textures
				Texture SkyTexture = new Texture(BitmapHelper.rescale(
						BitmapHelper.convert(getResources().getDrawable(
								R.drawable.sky)), 64, 64));
				TextureManager.getInstance().addTexture("SkyTexture",
						SkyTexture);
				Texture StarTexture = new Texture(BitmapHelper.rescale(
						BitmapHelper.convert(getResources().getDrawable(
								R.drawable.startexture)), 64, 64));
		...
		public void onDrawFrame(GL10 gl) {
			...
			// Screen Rotation Constant. This affects how fast the camera moves
			// when sliding a touch across the screen.
